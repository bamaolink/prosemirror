...
class BamaoLinkProseMirror {
  ...
  // 监听
  on(
    event: keyof BamaoLinkEditorType.Events,
  ...
  view: EditorView;
  options?: BamaoLinkEditorType.Options;
  ...
  // 创建
  create(dom: HTMLElement | string, options?: BamaoLinkEditorType.Options) {
    const app = typeof dom === "string" ? document.querySelector(dom) : dom;
    const { initialValue = "" } = options || {};
    const doc = this.markdownToNode(initialValue);
    const plugins = createPlugins(schema, emitter);
    ...
    // 添加样式
    view.dom.classList.add("bamao-link-prosemirror");
